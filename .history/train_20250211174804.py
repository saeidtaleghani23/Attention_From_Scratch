...
def run_val_epoch(model,  val_dataloader,loss_function, device, encoder_tokenizer, decoder_tokenizer, max_seq_len, results, epoch,prefix = 'val'):
    ...
    running_accuracy = []
    running_loss = []
    batch_iterator = tqdm(val_dataloader, desc=f"validation Processing epoch {epoch:02d}")
    with torch.no_grad():
        for batch in batch_iterator:
            ...
            # print('*'*50)
            # print('shape of the parameters in run_val_epoch')
...
def run_train_epoch(model,optimizer,train_dataloader,loss_function, device,results, encoder_tokenizer, epoch):
    ...
    running_loss = []
    running_accuracy = []
    batch_iterator = tqdm(train_dataloader, desc=f"Training Processing epoch {epoch:02d}")
    for batch in batch_iterator:
